...
class AjaxController extends Controller
{
    ...
    private function getOrderChat($Role_ID, Request $request)
    {
        ...
        if ($Role_ID == 6) {
            ...
            $query->whereHas('authorized_user', function ($q) {
                $q->whereIn('Role_ID', [1, 4, 5, 7, 8, 9, 10, 11, 12, 17]);
    ...
    public function getSearchingRecords(Request $request)
    {
        ...
        foreach ($orderInfo as $record) {
            ...
            $responseHtml .= '
            <td><strong>Creted By:</strong> ' . ($record->authorized_user->basic_info['full_name'] ?? '') . '</td>

            <td>   <a href="' . $ViewOrderRoute . '" class="btn btn-primary">View Order</a></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
