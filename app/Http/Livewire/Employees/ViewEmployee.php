        ...
        try {
            ...
            if ($request->EMP_Status == 1) {
                UserLeaveInfo::where('user_id', $emp_id)
                    ->update([
                        'Sick_Leaves' => $empLeaveInfo['Sick_Leaves'],
                        'Annual_Leaves' => $empLeaveInfo['Annual_Leaves'],
                        'Casual_Leaves' => $empLeaveInfo['Casual_Leaves'],
                        'Off_Day' => $empLeaveInfo['Off_Day']
                    ]);
                ...
                $annualQuota = $empLeaveInfo['Annual_Leaves'];
                $casualQuota = $empLeaveInfo['Casual_Leaves'];
                $sickQuota = $empLeaveInfo['Sick_Leaves'];
                $currentDate = new DateTime();
                $endOfYear = new DateTime($currentDate->format('Y-12-31'));
                $monthsWorked = $currentDate->diff($endOfYear)->y * 12 + $currentDate->diff($endOfYear)->m;
                $totalMonthsInYear = 12;
                $proratedAnnualQuota = intval(($monthsWorked / $totalMonthsInYear) * $annualQuota);
                $proratedCasualQuota = intval(($monthsWorked / $totalMonthsInYear) * $casualQuota);
                $proratedSickQuota = intval(($monthsWorked / $totalMonthsInYear) * $sickQuota);
                UserLeaveQuota::where('user_id', $emp_id)
                    ->update([
                        'Sick_Leaves' => $proratedSickQuota,
                        'Annual_Leaves' => $proratedAnnualQuota,
                        'Casual_Leaves' => $proratedCasualQuota,
                        'Off_Day' => $empLeaveInfo->Off_Day
                    ]);
            }
            DB::commit();
